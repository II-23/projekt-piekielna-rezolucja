...
class CreditsScene(BaseScene):
    ...
    def on_entry(self, *args, **kwargs):
        ...
        self.introduction = Credits_Text(os.path.join(soundtrack_authors_dir, "introduction"), 0.08 * self.width)
        self.soundtrack_section.append(self.introduction)
        self.ost_name_en = Credits_Text(os.path.join(soundtrack_authors_dir, "ost_name_en"), 0 * self.width)
        self.soundtrack_section.append(self.ost_name_en)
        self.ost_name_pl = Credits_Text(os.path.join(soundtrack_authors_dir, "ost_name_pl"), 0 * self.width)
        self.soundtrack_section.append(self.ost_name_pl)
        self.zostalstworzony = Credits_Text(os.path.join(soundtrack_authors_dir, "zostalstworzony"), 0 * self.width)
        self.soundtrack_section.append(self.zostalstworzony)
        self.najwiekszy = Credits_Text(os.path.join(soundtrack_authors_dir, "najwiekszy"), 0 * self.width)
        self.soundtrack_section.append(self.najwiekszy)
        self.polskizespol = Credits_Text(os.path.join(soundtrack_authors_dir, "polskizespol"), 0 * self.width)
        self.soundtrack_section.append(self.polskizespol)
        self.upamietniajacy = Credits_Text(os.path.join(soundtrack_authors_dir, "upamietniajacy"), 0 * self.width)
        self.soundtrack_section.append(self.upamietniajacy)
        self.najbardziej_znany = Credits_Text(os.path.join(soundtrack_authors_dir, "najbardziej_znany"), 0 * self.width)
        self.soundtrack_section.append(self.najbardziej_znany)
        self.parkwodny = Credits_Text(os.path.join(soundtrack_authors_dir, "parkwodny"), 0 * self.width)
        self.soundtrack_section.append(self.parkwodny)
        self.highdoosh = Credits_Text(os.path.join(soundtrack_authors_dir, "highdoosh"), 0 * self.width)
        self.soundtrack_section.append(self.highdoosh)
        self.wskladzie = Credits_Text(os.path.join(soundtrack_authors_dir, "wskladzie"), 0 * self.width)
        self.soundtrack_section.append(self.wskladzie)
        self.maks = Credits_Text(os.path.join(soundtrack_authors_dir, "maks"), 0 * self.width)
        self.soundtrack_section.append(self.maks)
        self.pluzin = Credits_Text(os.path.join(soundtrack_authors_dir, "pluzin"), 0 * self.width)
        self.soundtrack_section.append(self.pluzin)
        self.matpil = Credits_Text(os.path.join(soundtrack_authors_dir, "matpil"), 0 * self.width)
        self.soundtrack_section.append(self.matpil)
        self.tymon = Credits_Text(os.path.join(soundtrack_authors_dir, "tymon"), 0 * self.width)
        self.soundtrack_section.append(self.tymon)
        self.magda = Credits_Text(os.path.join(soundtrack_authors_dir, "magda"), 0 * self.width)
        ...
        self.guest_starred_introduction = Credits_Text(os.path.join(guest_starred_dir, "guests_introduction"), 0 * self.width)
        self.guest_starred_section.append(self.guest_starred_introduction)
        self.jma = Credits_Text(os.path.join(guest_starred_dir, "JMa"), 0 * self.width)
        self.guest_starred_section.append(self.jma)
        self.jmi = Credits_Text(os.path.join(guest_starred_dir, "JMi"), 0 * self.width)
        self.guest_starred_section.append(self.jmi)
        self.jmi2 = Credits_Text(os.path.join(guest_starred_dir, "JMi2"), 0 * self.width)
        self.guest_starred_section.append(self.jmi2)
        self.jmiimg = Credits_Image(os.path.join(ASSETS_DIR, "jmi.png"), 0, (0.4 * self.width, 0.4*self.height))
